...
//            alarmHelper.setUpAlarm(101, "test", calendar)
            ReminderManager.scheduleReminders(this, 5)
        }
    }
    ...
    override fun initData() {
        if (this.hasExactAlarmPermission() != true) {
            AlertDialog.Builder(this).setTitle(
                getString(R.string.set_exact_alarm_permission_title)).setMessage(
                getString(R.string.set_exact_alarm_permission_text)).setPositiveButton(
                getString(R.string.health_dialog_confirm)) { _, _ ->
