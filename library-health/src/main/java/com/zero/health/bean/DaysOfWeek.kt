...
data class DaysOfWeek(val coded: Int) {
    ...
    fun toString(context: Context, showNever: Boolean): String {
        return when {
            ...
            // count selected days
                ...
                (0..6).filter { it.isSet() }.map { dayIndex -> DAY_MAP[dayIndex] }.map { calDay -> dayStrings[calDay] }.joinToString(context.getText(R.string.day_concat))
            }
        }
    }
    ...
    override fun toString(): String {
        return (if (0.isSet()) "m" else "_") + (if (1.isSet()) 't' else '_') + (if (2.isSet()) 'w' else '_') + (if (3.isSet()) 't' else '_') + (if (4.isSet()) 'f' else '_') + (if (5.isSet()) 's' else '_') + if (6.isSet()) 's' else '_'
    }
